...
export function ThemeProvider({ children }: { children: ReactNode }) {
    ...
    const initialTheme = storedTheme ? themeOptions[storedTheme] : themeOptions.default;
    const [theme, setTheme] = useState<{ light: object; dark: object }>(initialTheme);

    useEffect(() => {
        const currentThemeKey = Object.keys(themeOptions).find(key => themeOptions[key] === theme);
        if (currentThemeKey) {
